...
function setMap(interval, speed, options) {
  ...
  if (step == null) {
    ...
    options.response.emit('update', { stepCoords:step.coords, pitch:pitch, zoom:zoom, stepSpeed:step.speed, stepTime:step.time, options:options }); // pass the new parameters back for display on the map
  }
