...
function setMap(interval, speed, options) {
  ...
  if (step == null) {
    ...
    options.response.emit('update', { pitch:pitch, zoom:zoom, stepCoords:step.coords, stepTime:step.time, stepSpeed:step.speed, options:options }); // pass the new parameters back for display on the map
  }
