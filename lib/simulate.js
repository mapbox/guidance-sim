...
function simulate(map, config, version) {
  ...
  setTime(emitter, scrub); // modify the emitter start time accordingly
  var speed = util.speed(config.speed, frequency);

  var center = getStep(config.route, scrub, version, 'coords');
  ...
  var interval = setInterval(function () { // at each interval, determine new parameters and ease map
    setMap(interval, step, emitter, config, version, maneuvers, map, speed, response, frequency);
  }, speed);
  ...
  function setSpeed(interval, speed) {
...
function setMap(interval, step, emitter, config, version, maneuvers, map, speed, response, frequency) {
  step = emitter.next();
  if (step == null) {
    response.emit(clearInterval(interval));
  } else {
    ...
    response.emit('update', { coords:step.coords, pitch:pitch, zoom:zoom, speed:step.speed, emitter:emitter, frequency:frequency, time:step.time }); // pass the new parameters back for display on the map
  }
  return response;
}
