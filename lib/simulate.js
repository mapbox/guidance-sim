...
function simulate(map, config, version) {
  ...
  var interval = setInterval(function () { // at each interval, determine new parameters and ease map
    setMapStuff(interval, step, emitter, config, version, maneuvers, map, speed, response, frequency);
  }, speed);
...
module.exports.setMapStuff = setMapStuff;
function setMapStuff(interval, step, emitter, config, version, maneuvers, map, speed, response, frequency) {
  step = emitter.next();
  if (step == null) {
    response.emit(clearInterval(interval));
  } else {
    ...
    response.emit('update', { coords:step.coords, pitch:pitch, zoom:zoom, speed:step.speed, emitter:emitter, frequency:frequency }); // pass the new parameters back for display on the map
  }
  return response;
}
