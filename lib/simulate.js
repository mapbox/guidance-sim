...
function simulate (map, config, version) {
  ...
  var start = seek(config, route); // fast-forward if applicable
  map.setCenter(route.geometry.coordinates[start]); // find the new first coordinate and center the map around it

  var frequency = 1000; // set frequency and invoke the event emitter
  var emitter = new Emitter(route, frequency, start);
...
/**
 * This function determines where the user wants to begin the simulation in the
 * properties.coordinateProperties.times array, given a start time in milliseconds.
 * @name seek
 * @param {Object} config Configuration parameters. Must have a `time` property.
 * See `run` function for more information.
 ...
 * @returns {Number} seek or index in times array.
 */
